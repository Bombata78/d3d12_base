...
//    Constructor
////////////////////////////////////////////////////////////////////////////////////////////////////
IOMesh::IOMesh(D3D12_PRIMITIVE_TOPOLOGY topology_, IOBuffer&& vertexBuffer_, IOBuffer&& indexBuffer_, std::vector<IOMesh::Attribute>&& attributes_)
    : topology(topology_), 
...
IOMesh* LoadObj(const std::string& filename, bool invertUVs)
{
    ...
    attributes.push_back(IOMesh::Attribute(IOMesh::Attribute::POSITION, DXGI_FORMAT_R32G32B32_FLOAT, 0));
    attributes.push_back(IOMesh::Attribute(IOMesh::Attribute::TEXCOORD, DXGI_FORMAT_R32G32_FLOAT,    GetByteStrideFromFormat(DXGI_FORMAT_R32G32B32_FLOAT)));
    attributes.push_back(IOMesh::Attribute(IOMesh::Attribute::NORMAL,   DXGI_FORMAT_R32G32B32_FLOAT, attributes[IOMesh::Attribute::TEXCOORD].offset + GetByteStrideFromFormat(DXGI_FORMAT_R32G32_FLOAT)));

    // Allocate the vertex/index buffers
    std::size_t vertexByteStride = attributes[IOMesh::Attribute::POSITION].offset + attributes[IOMesh::Attribute::TEXCOORD].offset + attributes[IOMesh::Attribute::NORMAL].offset;
    IOBuffer    vertexBuffer(vertexByteStride, objIndices.size()/3 * vertexByteStride);
    //DXGI_FORMAT indexFormat = (objIndices.size()/3 <= std::numeric_limits<std::uint16_t>::max()) ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT;
    DXGI_FORMAT indexFormat = (objIndices.size()/3 <= 0xFFFF) ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT;
    IOBuffer    indexBuffer(indexFormat, objIndices.size()/3 * GetByteStrideFromFormat(indexFormat));

    // Fill the vertex/index buffer
    std::uint8_t* pVertexData = reinterpret_cast<std::uint8_t*>(vertexBuffer.getData());
    std::uint8_t* pIndexData  = reinterpret_cast<std::uint8_t*>(indexBuffer.getData());
    for (std::uint32_t i = 0; i < objIndices.size(); i+=3)
    {
        ...
        pVertexData += GetByteStrideFromFormat(DXGI_FORMAT_R32G32B32_FLOAT);
        ...
        pVertexData += GetByteStrideFromFormat(DXGI_FORMAT_R32G32_FLOAT);
        ...
        pIndexData += GetByteStrideFromFormat(indexFormat);
    }

    return new IOMesh(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST, std::move(vertexBuffer), std::move(indexBuffer), std::move(attributes));
}